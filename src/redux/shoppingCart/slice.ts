...
export const checkout = createAsyncThunk("shoppingCart/checkout", async (jwt: string, thunkAPI) => {
	const { data } = await axios.post(`http://82.157.43.234:8080/api/shoppingCart/checkout`, null, {
		headers: {
			Authorization: `bearer ${jwt}`,
		},
	});
...
export const clearShoppingCartItem = createAsyncThunk(
	...
	async (parameters: { jwt: string; itemIds: number[] }, thunkAPI) => {
		return await axios.delete(`http://82.157.43.234:8080/api/shoppingCart/items/(${parameters.itemIds.join(",")})`, {
			headers: {
				Authorization: `bearer ${parameters.jwt}`,
			},
		});
